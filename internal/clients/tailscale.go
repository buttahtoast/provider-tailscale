...
import (
	...
	clusterv1beta1 "github.com/buttahtoast/provider-tailscale/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/buttahtoast/provider-tailscale/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		setIfPresent(ps.Configuration, creds, keyAPIKey)
		setIfPresent(ps.Configuration, creds, keyAudience)
		setIfPresent(ps.Configuration, creds, keyBaseURL)
		setIfPresent(ps.Configuration, creds, keyIdentityToken)
		setIfPresent(ps.Configuration, creds, keyIdentityTokenEnvVarName)
		setIfPresent(ps.Configuration, creds, keyOAuthClientID)
		setIfPresent(ps.Configuration, creds, keyOAuthClientSecret)
		setIfPresent(ps.Configuration, creds, keyTailnet)
		setIfPresent(ps.Configuration, creds, keyUserAgent)
		if v, ok := creds[keyScopes]; ok {
			var scopes []string
			if err := json.Unmarshal([]byte(v), &scopes); err == nil {
				ps.Configuration[keyScopes] = scopes
			}
		}
		return ps, nil
	}
...
func setIfPresent(configuration map[string]any, creds map[string]string, key string) {
