...
export TERRAFORM_PROVIDER_DOWNLOAD_URL_PREFIX ?= https://github.com/tailscale/terraform-provider-tailscale/releases/download/v$(TERRAFORM_PROVIDER_VERSION)
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-tailscale_v$(TERRAFORM_PROVIDER_VERSION)
...
REGISTRY_ORGS ?= xpkg.upbound.io/buttah-cloud
IMAGES = $(PROJECT_NAME)
...
XPKG_REG_ORGS ?= xpkg.upbound.io/buttah-cloud
# NOTE(hasheddan): skip promoting on xpkg.crossplane.io as channel tags are
# inferred.
XPKG_REG_ORGS_NO_PROMOTE ?= xpkg.upbound.io/buttah-cloud
XPKGS = $(PROJECT_NAME)
